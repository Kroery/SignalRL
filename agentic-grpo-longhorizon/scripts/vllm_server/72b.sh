...
MODEL_PATH=${MODEL_PATH:-"/mnt/data/lwy/code/vlm_models/Qwen2.5-72B-Instruct-AWQ"}
PORT=${PORT:-8001}               # policy 用 8000，user sim 用 8001
TP_SIZE=${TP_SIZE:-1}            # 单卡 A800 80GB 跑 72B AWQ，TP=1 足够
GPU_MEM_UTIL=${GPU_MEM_UTIL:-0.92}  # 16K 上下文 + AWQ，0.90 留安全余量
MAX_MODEL_LEN=${MAX_MODEL_LEN:-8192}  # 16K 硬约束
MAX_NUM_SEQS=${MAX_NUM_SEQS:-8}        # policy 默认 4；user sim 上下文短，可设 6-10
CUDA_DEVICES=${CUDA_DEVICES:-3}  # GPU 0 和 1 当 policy，GPU 2 当 user sim
